...
const fetchMovieDetails = async (movieId) => {
  const res = await fetch(`https://api.themoviedb.org/3/movie/${movieId}?api_key=fe59fa22c3ea09b2870a2f1e436c885f&language=en-US`);
  const data = await res.json();
  return data;
};
      ...
      >
        <div className="banner-bottom-content">
          ...
          <div className="banner-left">
            ...
            <h1 className="movie-title">{bannerMovie?.title || bannerMovie?.name}</h1>

            <div className="movie-info">
              <span>{bannerMovie?.release_date || "Date Unknown"}</span>
              <span> | {isDetailsLoading ? "Loading..." : `${Math.floor(movieDetails?.runtime / 60)}h ${movieDetails?.runtime % 60}m`} </span>
            ...
            <p className="banner-genres">
              {isDetailsLoading
                ? "#Loading Genres"
                : movieDetails?.genres?.map((genre) => `#${genre.name}`).join(' | ') || "#NoGenres"}
            </p>
